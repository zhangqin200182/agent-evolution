...
description: Launches a new CLI-1 session for rllm-train execution. Supports interactive mode (new Terminal window via osascript) and non-interactive mode (claude -p background). Ensures each training task gets a fresh session_id.
metadata:
  version: "1.0.0"
  categories:
    - trajectory
